...
2. Use the username and password from your own user_template.yaml file ([CloudHub/CloudHub-deploy/overlay/user_template.yml](../CloudHub-deploy/overlay/user_template.yml#L102))
...
If address `http://cluster_ip:30081` is not available, ensure that you used the `--with-elk` flag on cluster installation step ([GitHub - CloudHub/CloudHub](https://github.com/mspcloudopsengineer/awscloud/tree/integration?tab=readme-ov-file#cluster-update))  and `elk-0` pod in the running state.
